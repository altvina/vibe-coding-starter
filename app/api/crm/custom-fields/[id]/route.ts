...
export async function PATCH(
  ...
  let i = 1;
  ['label', 'field_type', 'options_json', 'is_required', 'order_index'].forEach((f) => {
    if (body[f] !== undefined) {
      setClauses.push(`${f} = $${i}`);
      values.push(body[f]);
      i++;
  ...
  try {
    const row = await withCrmDb(async (client) => {
      const r = await client.query(
        `UPDATE crm_custom_field_definitions SET ${setClauses.join(', ')} WHERE id = $${i} RETURNING *`,
        values
      );
...
export async function DELETE(
  ...
  const { id } = await params;
  try {
    const r = await withCrmDb(async (client) => client.query(`DELETE FROM crm_custom_field_definitions WHERE id = $1 RETURNING id`, [id]));
