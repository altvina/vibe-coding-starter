...
export async function PATCH(
  ...
  const fields = ['organization_id', 'first_name', 'last_name', 'email', 'phone', 'job_title'];
  ...
  try {
    const row = await withCrmDb(async (client) => {
      const r = await client.query(
        `UPDATE crm_people SET ${setClauses.join(', ')} WHERE id = $${i} RETURNING *`,
        values
      );
      return r.rows[0] ?? null;
    });
...
export async function DELETE(
  ...
  const { id } = await params;
  try {
    const r = await withCrmDb(async (client) => client.query(`DELETE FROM crm_people WHERE id = $1 RETURNING id`, [id]));
